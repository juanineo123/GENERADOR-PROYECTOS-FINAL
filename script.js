...
async function generateProjectBlock(blockName) {
    ...
    const requestData = { ...formData, block: blockName, problem: selectedProblem };

    try {
        const response = await fetch('/.netlify/functions/generate-project-block', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        alert(`Ocurrió un error al generar esta parte del proyecto: ${error.message}`);
        currentStep = 3; 
        showStep(currentStep);
        updateProgressBar();
    }
